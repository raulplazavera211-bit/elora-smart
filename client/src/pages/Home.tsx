...
function EloraContactMap({ className }: { className?: string }) {
  return (
    <div className={`relative overflow-hidden bg-[#f5f5f0] ${className ?? ""}`}>
      <iframe
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-1/2 z-10 flex -translate-x-1/2 -translate-y-full flex-col items-center drop-shadow-[0_4px_14px_rgba(217,119,6,0.42)]"
      >
        <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border-[3px] border-[#d97706] bg-white shadow-lg">
          <img src={LOGO_URL} alt="" className="h-9 w-9 object-contain" />
        </div>
        <span className="-mt-1.5 h-3 w-3 rotate-45 border-b-[3px] border-r-[3px] border-[#d97706] bg-white" />
      </div>
    </div>
  );
}
